...
def color_label(val):
    if val == 0:
        return f"<span style='color:green;font-weight:bold'>{val} (Safe)</span>"
    elif val == 1:
        return f"<span style='color:orange;font-weight:bold'>{val} (Intermediate)</span>"
    else:
        return f"<span style='color:red;font-weight:bold'>{val} (Resistant)</span>"
...
def predict(location, imipenem, ceftazidime, gentamicin, augmentin, ciprofloxacin):
    ...
    # =========================
    try:
        img = mpimg.imread("accuracy_graph.png")
...
""")
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            plot3 = gr.Plot(label="Model Accuracy")
    ...
    btn.click(
        ...
        outputs=[output, plot1, plot2, plot3, pdf_output]
    )

    clear.click(
        lambda: (20, 20, 20, 20, 20, "", None, None, None, None),
        outputs=[imipenem, ceftazidime, gentamicin, augmentin, ciprofloxacin, output, plot1, plot2, plot3, pdf_output]
    )
